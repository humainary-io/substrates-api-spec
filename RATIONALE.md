...
**Companion to SPEC.md Version 3.6.0**
...
### Why Processing Order, Not Admission Position

§7.6.1 defines visibility over the circuit's logical processing order rather than over the position
at which work was admitted. Admission position is the more intuitive coordinate, and it is the one
the rule reads like in the common case, so it is worth saying why it is not the definition.

Admission position is a property that only ingress work has. A cascade's work is generated inside
the circuit context and is never admitted from anywhere; the obvious repair — letting cascading
work inherit the position of the ingress item it descends from — fails as soon as a cascade
contains a topology change. Suppose a callback emits, subscribes, and emits again. Transit then holds
`[E1, register(S), E2]`, and processing `E1` appends `E1a` behind `E2`. Under inheritance all four
operations descend from one ingress item and share one position, which orders none of them against
each other. Under processing order the answer is immediate: `S` is effective once `register(S)`
runs, so it misses `E1` and catches `E2` and `E1a`.

That last part is the substantive point rather than a technicality. `E1a` is caused by `E1`, which
`S` never saw, and `S` sees `E1a` anyway. Visibility is not inherited along causal ancestry; it is
decided by the subscriptions in effect when a channel processes an emission. Emissions can share one
stimulus reading (§5.8) without sharing one topology, and the two notions must not be conflated.

The same reasoning fixes the class of a subscribe or close issued from inside a callback. §5.3
classifies queued work by the context that submitted it, so such a call is transit work on the
owning circuit and takes effect within the current cascade — which is what makes "all effects of
an emission complete before the next external emission begins" true of topology changes and not
only of emissions. Classifying it as ingress would have carved an exception out of that property for no
gain.

Cancellation is deliberately weaker than visibility, and for the same reason the two are separated:
a close ends future *selection*, and cannot retract a delivery already dispatched. Making close
retroactive would mean either scanning queued work for deliveries to withdraw, or holding a
dispatch open until every recipient has run — the first is an unbounded scan on the hot path, the
second reintroduces the synchronous call chain the admission boundary exists to break (§3).
