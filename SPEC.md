...
- Deterministic replay is achievable by replaying the circuit's admitted operation sequence (see
  ...
  determines how subsequent emissions are routed. Replaying operations alone is also insufficient
  wherever time-aware operators are present, because they observe stimulus time (§5.8), which is
  read from a clock rather than carried by the operations. A replay log MUST record the stimulus
  time each ingress item observed, or the replay MUST run against a clock that supplies those
  readings. `chance` (§6.2.3) draws from an implementation-defined random source, so replay
  reproduces its decisions only where the implementation documents a way to seed that source; an
  implementation that provides none MUST document `chance` as outside what replay reproduces.
...
A queued operation's class is decided by the **context it was submitted from**, not by what kind of
operation it is. Both classes carry emissions and non-emission operations alike.

**Ingress work** is work accepted from a caller context (§5.2) — that is, from any context other
than the owning circuit's own. This includes emissions and other queued operations such as
subscription registration, subscription close, and resource cleanup work. Work submitted from
*another* circuit's context is a caller context from the receiving circuit's point of view, and is
therefore ingress work to it.

**Transit work** is work generated while the owning circuit context is already processing. The
cascading emissions raised by processing an item are transit work, and so is every other queued
operation raised there: a subscription registration, a subscription close, or cleanup work submitted
from within a callback joins transit on the same terms as an emission.
...
When transit work itself submits further work, that work is sequenced behind transit work already
accepted for the current cascading chain. Transit is FIFO over every kind of operation it carries,
not over emissions alone. The model is always iterative, never recursive.
...
Because stimulus time is read from a clock rather than carried by the admitted operations, a replay
(§5.1) must record or supply it.

This model favors causal consistency and aggregation (turning many co-temporal signals into a
...
(created via `conduit.get(name)`) and circuit-created pipes submit work to their owning circuit,
which then dispatches it to the named pipe's registrations or to the target pipe or receptor.
Registering a pipe does not change how it delivers: a named pipe dispatching to a registered pipe
emits to it, and the registered pipe's delivery then follows its own contract (§6.3).
...
- Pipes registered with a named pipe are emitted to when the named pipe dispatches. Their delivery
  then follows their own contract, as the two cases above describe (§6.3).
...
independent of output). Derivative-style computations (deltas, differences, two-point smoothing)
...
After an emission passes through all fiber/flow stages, the named pipe dispatches it to its
registrations: the downstream pipes and receptors registered with it (§7.4). Dispatch visits the
registrations sequentially, in registration order, within the circuit context. The two kinds are
delivered differently:

- A **receptor registration** is invoked at its place in that order, during the dispatch. Its
  receive operation completes before dispatch moves on to the next registration.
- A **pipe registration** is emitted to at its place in that order. The emission then follows the
  pipe's own contract: a pipe owned by the dispatching circuit accepts it as transit work (§5.3),
  sequenced behind work already accepted, and a pipe owned by another circuit accepts it as ingress
  work to that circuit.

A receptor registration is therefore a direct call and a pipe registration a queued delivery, and a
registrant chooses between them by kind. An implementation MUST NOT queue a receptor registration,
and MUST NOT deliver a pipe registration by invoking the pipe's target directly: either changes the
order in which registrants observe the emission relative to other accepted work. For example, if a
channel has a pipe registration `P` followed by a receptor registration `R`, and transit work `W` was
accepted before the channel dispatches, the registrants observe `R`, then `W`, then `P`'s delivery.

Receptor registrations have the same channel visibility, temporal validity, circuit-context
confinement, and failure-isolation guarantees as pipe registrations. An exception thrown by any one
...
**Callback-scoped**: Valid only during a specific callback invocation, and only on the circuit
context that issued it. The object MUST NOT be retained after the callback returns, and MUST NOT be
used from any other execution context, including one the callback itself starts, even while the
...
Using a temporal object beyond its lifetime is an **illegal operation**. So is using a
callback-scoped object from an execution context other than the circuit context that issued it:
that use is handled by the same rules as a use after the callback returns, including the detection
requirements below. Implementations MUST handle violations in one of the following ways:
...
Implementations MAY reuse the storage behind a callback-scoped object across callbacks, such as a
Window's rolling buffer, but not the object handed to a callback: a reference handed to one callback
MUST NOT become valid again in a later one. Detection depends on this. A retained reference revived
for a later callback would be indistinguishable from the one legitimately handed to that callback,
so its misuse could not be detected. A Window observed during a callback
...
3. When a named pipe within the source processes an emission at a position within the
   subscription's visibility window (§7.6), and the named pipe has not yet rebuilt for this
   subscription, a rebuild is triggered within the circuit context.
...
- Registrations of both kinds are dispatched in registration order. A receptor registration is
  invoked during dispatch; a pipe registration is emitted to, and its delivery follows the pipe's
  own contract (§6.3).
...
- Does **not** retract deliveries already selected. Work dispatched to this subscription's pipes
  before the close job executed MUST still be performed (§7.6.1).
...
A close MAY be requested from within a subscriber callback or a registered receptor. Doing so
submits the close job from the owning circuit's own context, which makes it transit work taking
effect within the current cascade (§5.3, §7.6.1); the same call from any other context is ingress
work and takes effect after the cascade has drained. In neither case does it cancel deliveries the
current emission has already selected.
...
A circuit's **logical processing order** is the single sequence in which its context processes the
work it has accepted: transit work ahead of ingress work, each class FIFO within itself (§5.3). It
is the order in which operations take effect, and it is the coordinate this section is defined in.
Admission position is not, because a cascade's work has no admission position of its own.

A subscription's **visibility window** is the half-open interval of that order running from the
point its registration job **executes** to the point its close job executes (or unbounded if it has
not been closed). A subscription is **effective** at the positions inside its window.

When a channel processes an emission, it selects as recipients the registered pipes of exactly those
subscriptions effective at that position. An emission a channel processes before a subscription's
registration job, or at or after its close job, does not select that subscription.

Three consequences are stated explicitly, because they are what the rule decides:

- **Selection and execution are distinct, and cancellation reaches only the first.** Closing a
  subscription removes it from selections made at later positions. Deliveries already selected and
  queued MUST still be performed. A close does not retract dispatched work, and a subscriber that
  closes its own subscription from within one of its receptors does not thereby cancel the sibling
  deliveries that the same emission already selected.
- **A topology change inside a cascade takes effect at its own position**, not at the position of
  the ingress item the cascade descends from. Suppose transit holds `[E1, register(S), E2]`.
  Processing `E1` raises `E1a`, which joins the back of transit, giving
  `[register(S), E2, E1a]`. `S` is effective from the point `register(S)` executes, so `S` does not
  see `E1`, and does see both `E2` and `E1a` — even though `E1a` descends from `E1`. Causal
  ancestry does not determine visibility; the subscriptions effective when a channel processes an
  emission do. All three emissions may share one stimulus time (§5.8) without sharing one topology.
- **Classification follows the submitting context** (§5.3). A subscribe or close issued from within
  a callback is transit work on the circuit that owns the source, and takes effect within the
  current cascade. The same call issued from any other context — including another circuit's — is
  ingress work, and takes effect after the current cascade has drained.

**The ingress-only special case.** Where every operation concerned is submitted from caller
contexts, no transit work intervenes, and the window reduces to an interval of admission positions,
which is how it is most often reasoned about. A single caller context that executes
`subscribe(); emit(X); await()` observes `X` on the new subscriber: by per-caller FIFO the subscribe
job is admitted first, the circuit processes them in that order, and the registration executes
before `X` is dispatched. Conversely `emit(X); subscribe(); await()` does not observe `X`.
Symmetrically for close: `subscribe(); emit(X); close(); emit(Y); await()` delivers `X` but not `Y`.
...
visibility rule still applies — each subscription's window is defined by its own registration and
close positions — but the set of emissions falling inside a given window may differ across runs
when multiple callers are racing.
...
with minimal coordination overhead. The visibility rule in §7.6.1 is preserved because the version
increment is itself an operation in the circuit's logical processing order, taking effect at its own
position like any other — including when the subscription change was raised from within a cascade,
where that position lies inside the cascade rather than behind it.
...
- **Queued post-close semantics**: Pure queued operations that do not need to return a newly-created
  ...
  position in the circuit's logical processing order (§7.6.1) falls strictly after the component's
  own close job MUST be silently dropped by the component when processed on the circuit context: no
  delivery, no topology change, and no observable effect beyond any onClose-style callback symmetry
  defined by the specific operation type. The component MAY additionally report the drop via a side
  channel (e.g., a fault emission or a circuit-state event on the circuit context). Operations
  positioned strictly *before*
...
- **Open-required synchronous semantics**: Operations that create resources, create topology, return
  ...
  operations as open-guarded. The error MUST identify the subject of the receiver whose guard
  rejected the call. Where a guarded operation is satisfied by invoking another guarded operation on
  a different receiver — for example, `Bank.get` materializing a resource through its owning
  circuit's factory — the rejection is raised and attributed by the receiver that observed itself as
  non-open; an implementation MUST NOT re-attribute such an error to the outer receiver. When
  rejection is caused by a closed substrate argument, such as subscribing with a closed subscriber,
  the error MUST also identify that argument's subject.
...
Leaf-first governs the order in which dispatch visits each level's registrations: every
registration of the target named pipe before any of its parent's, and so on upward. Each
registration is then delivered as §6.3 describes. A receptor registered on an ancestor is therefore
invoked during the dispatch, ahead of the queued delivery of a pipe registered on the leaf.
...
has accepted close MUST signal a closed-resource error synchronously, identifying the bank's
subject.
A bank whose owning circuit has closed while the bank itself has not is not thereby closed: a lookup
for an already-materialized name MAY still return it, and a lookup that must materialize a new
resource is rejected by the circuit's own factory guard, which identifies the circuit rather than
the
bank (§9.1).
...
positive capacity; an invalid capacity (< 1) MUST be signaled synchronously as an invalid-argument
...
| **Illegal temporal use** | SHOULD detect (MUST detect for Registrar) | A temporal object (§6.4) is used after its callback scope has ended, or a callback-scoped object is used from an execution context other than the one that issued it. Detection is RECOMMENDED in general; undefined behavior is acceptable where detection would degrade valid-path performance. **Registrar violations MUST be detected and signaled** — see §6.4.1.                                                                                                                               |
...
For a closed-resource error, the signaled error MUST identify the subject of the receiver whose
guard rejected the call, as specified in §9.1. If the failed operation was rejected because a
substrate argument was closed, the error MUST also identify that argument's subject. Projections MAY
expose this as structured call metadata, an exception payload, an error value, or an equivalent
mechanism.
...
- A `Receptor` given to `Circuit.pipe`
...
- A function passed to `Port.update`
...
4. **Observability**. Implementations MAY accept a structured fault receptor when a circuit is
   created.
   When supplied, that receptor receives every isolated external-callback failure, attributed to the
   closest meaningful substrate subject known at the isolation boundary and otherwise to the
   circuit.
   The structured fault retains the original failure as its cause. A failure raised by the fault
   receptor is isolated and is not reported recursively. Without a supplied receptor,
   implementations
   MAY use a documented default reporting mechanism; applications SHOULD select explicitly when
   observation matters.
...
operations create a Bank of conduits (§10.4): the no-argument and type-classifier-only forms use
default per-pipe routing; the routing forms select the routing behavior. As with `conduit`, the
type classifier MAY be implicit — inferred by the projection from the call-site context — so a bank
whose emission type cannot be named by a class literal remains reachable. Each call returns a
distinct Bank
...
circuit-level overloads. The discarding form — one supplying neither targets nor a receptor —
returns a circuit-owned pipe whose dispatched emissions are discarded on the circuit context — work
is scheduled but the resulting job is a
...
forwarder). A named discarding pipe is also available directly as the name-only form, which is
equivalent to supplying an empty target list. Use a single-element list when a named single-target
forwarder is required. List-order
...
cell and is observed by readers until the first accepted update is processed. The named form
additionally assigns an explicit subject name; the unnamed form uses the owning circuit's name.
Cells have no absent state — there is no seedless form. The `basin` operation creates a
...
initial value, may be empty, and have no lifecycle of their own beyond the owning circuit. A basin
MAY be created with an explicit name; when no name is supplied, the implementation uses the owning
circuit's name. The
`port`
...
The named form additionally assigns an explicit subject name; the unnamed form uses the
owning circuit's name. Ports have no absent state — there is no seedless form. The `pin`
...
illegal-context-use error (§15.1). The named form additionally assigns an explicit subject
name; the unnamed form uses the owning circuit's name. Pins have no absent state — there is no
seedless form. The
`ticker` operations create circuit-owned periodic emitters (§11.4): both forms take a strictly
positive interval and a target pipe; the named form additionally assigns an explicit name,
while the unnamed form uses the owning circuit's name. The
...
(§11.1); the emission type is taken from the endpoint, and the named form additionally
...
`register(pipe)` attaches a downstream pipe, which dispatch emits to; its delivery follows the
pipe's own contract. `register(receptor)` attaches a terminal receptor, which dispatch invokes
directly. Registrations of both kinds are dispatched in registration order and have the same channel
visibility, temporal validity, circuit-context confinement, and failure-isolation guarantees (§6.3).
...
- `Fault` for synchronously detected substrate runtime errors and asynchronously reported isolated
  callback failures. `Fault.subject()` returns the closest known receiver/context subject,
  defaulting
...
Registrar violations are detected via sentinel state and an owner-thread check. Since fibers and flows are standalone builder
...
entry, which also compares the calling thread with the one that received the view. Detection
lands on the user-callback side, not the framework emission hot path. The Java projection permits
providers to back Window with mutable rolling storage reused across callbacks; each callback
receives its own view, whose lease the check tests.
