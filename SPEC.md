...
9. **Closed-resource faults**: Open-required operations invoked on a resource that has accepted
   close MUST signal a closed-resource error synchronously and identify the receiver subject (§9.1,
   §15.3).
10. **Scope ordering**: Resources registered with a scope MUST close in reverse registration order.
11. **State immutability**: State operations MUST preserve immutability — prior instances MUST
...
12. **Happens-before guarantees**: Implementations MUST provide the happens-before relations defined
    in §5.4.1 through appropriate synchronization mechanisms.
13. **Illegal circuit-context calls**: Calling `await`, `pulse`, or `closeAwait` from within the
...
14. **External callback isolation**: Uncaught exceptions from client-supplied callbacks (receptors,
...
15. **Subscriber callback consumption**: A subscriber callback is invoked exactly once for each
...
16. **Operator state ownership**: Client-supplied emission-time Fiber and Flow operator functions
...
17. **Bank resource identity**: For any name within a bank, repeated `Bank.get(name)` calls MUST
...
18. **Ticker scheduling**: A ticker MUST schedule ticks at a fixed rate — a late tick MUST NOT shift
